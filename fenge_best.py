...
def reset():     #重置所有计数器及容器 —— 拟合一次后调用
    ...
    return track_cur,track_cur_img,move_cur,move_cur_img,update_flag,\
           ...
           out_count,is_out_count_over,out_direction_dectect_flag,track_with_two_position,track_with_two_position_img
...
if __name__ == "__main__":
    ...
    fit_change_num_x_minus = 0      # 拟合点x负方向变化计数
    fit_change_num_y_minus = 0      # 拟合点y负方向变化计数
    fit_change_th = 15              # 拟合变化计数阈值

    in_direction_count = 0               # 处于该方向的次数
    in_direction_count_th = 5            # 处于该方向的次数阈值
    out_direction_count = 0              # 该方向确定之后，不在该方向的次数
    out_direction_count_th = 10          # 不在该方向上次数阈值
    out_direction_change_th = 20         # 不在该方向上变化计数阈值

    out_count = 0                   # 跑出有效区域个数
    out_count_th = 10               # 区域外次数阈值
    cur_area_count = 0              # 区域重合计数
    per_area_count = 0              # 上一帧区域重合计数
    area_cut_flag = False           # 出重叠区域后分段
    area_in_flag =  True          # 进入重叠区域
    is_out_count_over = False       # 代表是区域外计数足够了
    init_notin_flag  = False         #初始是否处于重叠区域，默认在
    out_direction_dectect_flag = False     # 其他方向计数开关

    position_dict = {'left': False, 'right': False, 'up': False}    #位置重合区域字典
    per_position_dict= {'left': False, 'right': False, 'up': False} #前一时刻位置重合区域字典
    match_dict = {'left':           #边框匹配字典
                    {
    ...
    done_dict = {'left':0,'right':0,'up':0}    #结果字典
    ...
    match_buffer = 40     #边框长度与轨迹长度匹配扩展距离
    ...
    for image_path,track_point in zip(img_file_list,track_xy):
        ...
        if not track_match_dict:
            track_match_dict = copy.deepcopy(match_dict)       #原始边框需要匹配的信息
        ...
        if is_in_area(track_point,box_buffer,box_in_buffer):          #有效区域内
            ...
            cur_area_count,position_dict = count_position_area(track_point,box_buffer,box_in_buffer)
            print("cur_area_count:",cur_area_count)

            if cur_area_count == 2 and per_area_count == 1:
                area_in_flag = True
                area_cut_flag = False
            if cur_area_count == 1 and per_area_count == 2: #只有第一次跑出重叠区域，重叠区域分段的条件才开启
                area_cut_flag = True
                if init_notin_flag:
                    area_in_flag = False
            if cur_area_count==1:    #只有第一次跑出重叠区域，重叠区域分段的条件才开启
                init_notin_flag = True
            if cur_area_count==2 and init_notin_flag:
                track_with_two_position.append(track_point)
                track_with_two_position_img.append(image_path)

            if diff_x > out_direction_change_th:       #往x正方向变化——分段找其他方向的结束点阈值
                change_direction = 'x_plus'
            if diff_x > fit_change_th:             #往x正方向变化——分段找该方向阈值
                fit_change_num_x_plus+=1

            if diff_x < -out_direction_change_th:      #往x负方向变化——分段找其他方向的结束点阈值
                change_direction = 'x_minus'
            if diff_x < -fit_change_th:            #往x负方向变化——分段找该方向阈值
                fit_change_num_x_minus+=1

            if diff_y > out_direction_change_th:       #往y正方向变化——分段找其他方向的结束点阈值
                change_direction = 'y_plus'
            if diff_y > fit_change_th:             #往y正方向变化——分段找该方向阈值
                fit_change_num_y_plus+=1

            if diff_y < -out_direction_change_th:      #往y负方向变化——分段找其他方向的结束点阈值
                change_direction = 'y_minus'
            if diff_y < -fit_change_th:            #往y负方向变化——分段找该方向阈值
                fit_change_num_y_minus+=1

            track_cur.append(track_point)
            track_cur_img.append(image_path)
            track_point_per = track_point
            per_position_dict = position_dict
            per_area_count = cur_area_count
        else:                                #有效区域外
            out_count+=1
            if per_area_count == 2:
                area_cut_flag = True
        ...
        if out_count>=out_count_th:
            ...
            if direction and track_cur:
                ...
                # 确定位置
                if 'y' in direction:
                    position = find_position(track_cur[-2],box_buffer,box_in_buffer) #用有效轨迹的最后一个点判断位置，防止最后一个轨迹点是无效的
                else:
                    position = 'up'
                ...
                track_minpoint_pro, track_maxpoint_pro, track_length_pro = projection_length(track_minpoint, track_maxpoint,
                                                                                             match_dict[position]['k_fit'], match_dict[position]['b_fit'])
            ...
            if update_flag:
                if direction == 'x_plus':
                    if track_match_dict[position][direction]['end_point']:
                        if track_x_max > track_match_dict[position][direction]['end_point']:    #之前匹配过切比原来的远
                            track_match_dict[position][direction]['end_point'] = track_x_max
                            track_match_dict[position][direction]['match_length'] += track_length_pro
                    else:
                        track_match_dict[position][direction]['end_point'] = track_x_max        #之前没有匹配过
                        track_match_dict[position][direction]['match_length'] = track_length_pro

                elif direction == 'x_minus':
                    if track_match_dict[position][direction]['end_point']:
                        if track_x_min < track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                            track_match_dict[position][direction]['end_point'] = track_x_min
                            track_match_dict[position][direction]['match_length'] += track_length_pro
                    else:
                        track_match_dict[position][direction]['end_point'] = track_x_min        #之前没有匹配过
                        track_match_dict[position][direction]['match_length'] = track_length_pro

                elif direction == 'y_plus':
                    if track_match_dict[position][direction]['end_point']:
                        if track_y_max > track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                            track_match_dict[position][direction]['end_point'] = track_y_max
                            track_match_dict[position][direction]['match_length'] += track_length_pro
                    else:
                        track_match_dict[position][direction]['end_point'] = track_y_max
                        track_match_dict[position][direction]['match_length'] = track_length_pro
                    ...
                    if track_match_dict[position][direction]['end_point']:
                        if track_y_min < track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                            track_match_dict[position][direction]['end_point'] = track_y_min
                            track_match_dict[position][direction]['match_length'] += track_length_pro
                    else:
                        track_match_dict[position][direction]['end_point'] = track_y_min
                        track_match_dict[position][direction]['match_length'] = track_length_pro  #同个方向的都拼一块了，没有判断起点终点！！！！！
            ...
            # 清除
            area_cut_flag = False
            isdone(match_dict, track_match_dict, done_dict,match_buffer)
            track_all.append(track_cur)
            track_img_all.append(track_cur_img)
            track_direction_all.append(direction)
            track_cur,track_cur_img, move_cur,move_cur_img,  update_flag, \
            position, direction, change_direction, in_direction_count, out_direction_count, \
            out_count, is_out_count_over, out_direction_dectect_flag,track_with_two_position,track_with_two_position_img = reset()

        else:
            direction_tmp = find_direction_whenin(fit_change_num_x_minus, fit_change_num_x_plus,
                                               fit_change_num_y_minus,fit_change_num_y_plus,in_direction_count_th)
            if not out_direction_dectect_flag:
                direction = copy.deepcopy(direction_tmp)
            print('direction:', direction,end=' ')
            if direction and is_in_area(track_point,box_buffer,box_in_buffer):
                out_direction_dectect_flag = True
                fit_change_num_x_plus, fit_change_num_y_plus, fit_change_num_x_minus, fit_change_num_y_minus = reset_direction_count([],out_direction_change_th)
                if out_direction_dectect_flag:
                    if abs(diff_x) > out_direction_change_th and abs(diff_y) > out_direction_change_th:  # 两个方向同时变就跳过去
                        out_direction_count += 1
                        move_cur.append(track_point)  # 被移除的轨迹点——下一段轨迹的开头
                        move_cur_img.append(image_path)
                    else:
                        if direction == change_direction:      # 后续出现该方向的其他点也归并到这一段里面
                            ...
                            move_cur.append(track_point)  # 被移除的轨迹点——下一段轨迹的开头
                            move_cur_img.append(image_path)
            # if (out_direction_count >= out_direction_count_th) and direction:
            # if ((out_direction_count>= out_direction_count_th) or area_cut_flag) and direction:
            # if ((out_direction_count >= out_direction_count_th) and not area_in_flag) and direction:
            # if (((out_direction_count >= out_direction_count_th)or area_cut_flag)and not area_in_flag) and direction:
            if (area_cut_flag and not area_in_flag) or ((out_direction_count >= out_direction_count_th) and direction):
                update_flag = True
                if not area_cut_flag:
                    track_cur = [point_xy for point_xy in track_cur if not np.isin(point_xy,move_cur).all()]    #移除下一段的轨迹点
                    track_cur_img = [img_point for img_point in track_cur_img if img_point not in move_cur_img]
                # 确定位置
                if direction:
                    if 'y' in direction:
                        position = find_position(track_cur[-2],box_buffer,box_in_buffer)
                    else:
                        position = 'up'
                # 拟合
                if position:
                    ...
                    track_minpoint_pro, track_maxpoint_pro, track_length_pro = projection_length(track_minpoint,
                                                                                                 track_maxpoint,
                                                                                                 match_dict[position]['k_fit'], match_dict[position]['b_fit'])
                    print("真实轨迹长度：", track_true_length, "投影长度：", track_length_pro,'Position:',position)
                    cv2.line(img_copy, track_minpoint, track_maxpoint, (255, 255, 0), 2)
                ...
                move_cur_img_tmp = copy.deepcopy(move_cur_img)

                if not area_cut_flag:
                    turn_index = find_turn_index(track_cur, direction)
                    move_cur_tmp = track_cur[turn_index+1:]+ move_cur_tmp
                    move_cur_img_tmp = track_cur_img[turn_index + 1:] + move_cur_img
                ...
                track_with_two_position_tmp = copy.deepcopy(track_with_two_position)
                track_with_two_position_img_tmp = copy.deepcopy(track_with_two_position_img)
                track_cur, track_cur_img,move_cur,move_cur_img, update_flag, \
                position, direction, change_direction, in_direction_count, out_direction_count, \
                out_count, is_out_count_over, out_direction_dectect_flag,track_with_two_position,track_with_two_position_img = reset()
                if area_cut_flag:
                    track_cur = track_cur + track_with_two_position_tmp
                    track_cur_img = track_cur_img + track_with_two_position_img_tmp

                else:
                    track_cur = track_cur + move_cur_tmp
                    track_cur_img = track_cur_img + move_cur_img_tmp
                # track_cur = track_cur + move_cur_tmp
                # track_cur_img = track_cur_img + move_cur_img_tmp

                #重新记录原来列表的数
                if cur_area_count==2:
                    init_notin_flag = False
                fit_change_num_x_plus, fit_change_num_y_plus, fit_change_num_x_minus, fit_change_num_y_minus = reset_direction_count(
                    track_cur, out_direction_change_th)
                direction = find_direction_whenin(fit_change_num_x_plus, fit_change_num_y_plus, fit_change_num_x_minus, fit_change_num_y_minus,in_direction_count_th)
                track_with_two_position.clear()
                track_with_two_position_img.clear()
                move_cur_tmp.clear()
                move_cur_img_tmp.clear()
            area_cut_flag = False
        #---------------------------------------------------------------------------------------------------     模拟轨迹
