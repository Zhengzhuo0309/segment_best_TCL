...
# from otsu_2threshold import Otsu2Threshold
# # 读取图像
...
def egde_detect_biology(gray):
    ...
    # mask_er = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=6)
...
# 更新边框字典
def fit_screen(match_dict,screen_contours_best,box_buffer,box_in_buffer):
    scb = screen_contours_best[0].squeeze()
    screen_contours_best_part_xy, screen_contours_best_part, screen_contours_best_part_length = dividing_contours(scb,
                                                                                                                  box_buffer,
                                                                                                                  box_in_buffer)
    (screen_contours_best_left_xy, screen_contours_best_right_xy,
     screen_contours_best_up_xy) = screen_contours_best_part_xy
    ...
    match_dict['left']['k_fit'] = k_fit
    match_dict['left']['b_fit'] = b_fit
    ...
    match_dict['right']['k_fit'] = k_fit_r
    match_dict['right']['b_fit'] = b_fit_r
    ...
    match_dict['up']['k_fit'] = k_fit_u
    match_dict['up']['b_fit'] = b_fit_u
...
def find_direction_whenout(fit_change_num_x_minus,fit_change_num_x_plus,fit_change_num_y_minus,fit_change_num_y_plus):
    ...
    if direction_max:
        ...
        return ''
# 界内找轨迹变化方向
def find_direction_whenin(fit_change_num_x_minus, fit_change_num_x_plus,fit_change_num_y_minus,fit_change_num_y_plus,in_direction_count_th):
    if fit_change_num_x_minus>=in_direction_count_th:
        return 'x_minus'
    elif fit_change_num_x_plus>=in_direction_count_th:
        return 'x_plus'
    elif fit_change_num_y_minus>=in_direction_count_th:
        return 'y_minus'
    elif fit_change_num_y_plus>=in_direction_count_th:
        return 'y_plus'
    else:
        return ''

def find_turn_index(track_cur,direction):
    if direction == 'y_plus':
        index = np.where(np.array(track_cur)[:, 1] == np.array(track_cur)[:, 1].max())[0][-1]
    elif direction == 'x_plus':
        index = np.where(np.array(track_cur)[:, 0] == np.array(track_cur)[:, 0].max())[0][-1]
    elif direction == 'x_minus':
        index = np.where(np.array(track_cur)[:, 0] == np.array(track_cur)[:, 0].min())[0][-1]
    elif direction == 'y_minus':
        index = np.where(np.array(track_cur)[:, 1] == np.array(track_cur)[:, 1].min())[0][-1]
    else:
        index = None
    return index
#确定轨迹点位置
...
def count_position_area(point,box_buffer,box_in_buffer):
    x_inbuffer_left, x_inbuffer_right, y_inbuffer_up, y_inbuffer_down = box_in_buffer[0][0], box_in_buffer[1][0], \
                                                                        box_in_buffer[1][1], box_in_buffer[2][1]
    xbuffer_left, xbuffer_right, ybuffer_up, ybuffer_down = box_buffer[0][0], box_buffer[1][0], \
                                                            box_buffer[1][1], box_buffer[2][1]
    count = 0
    # position_dict = {'left':False,'right':False,'up':False}
    is_in_left = (xbuffer_left <= point[0]) & (point[0] <= x_inbuffer_left)
    is_in_right = (x_inbuffer_right <= point[0]) & (point[0] <= xbuffer_right)
    is_in_up = (ybuffer_up <= point[1]) & (point[1] <= y_inbuffer_up)
    # 不能用两个都不在的，因为边框可能实时变化
    if is_in_left:
       count+=1
       # position_dict['left'] = True
    if is_in_right:
        count+=1
        # position_dict['right'] = True
    if is_in_up:
        count+=1
        # position_dict['up'] = True
    return count

def isdone(match_dict:dict,track_match_dict:dict,done_dict:dict,match_buffer:int):
    #  判断长度
    positions_x = ['up']
    directions_x = ['x_plus','x_minus']
    positions_y = ['left','right']
    directions_y = ['y_plus','y_minus']
    for position in positions_x:
        for direction in directions_x:
            if track_match_dict[position][direction]['match_length'] and match_dict[position][direction]['need_length']:
                if match_dict[position][direction]['need_length'] - match_buffer<= track_match_dict[position][direction]['match_length']:
                    done_dict[position] += 1  # 画好一条边
                    print('Done' + position)
                    print(done_dict)
                    track_match_dict[position][direction]['end_point'] = None
                    track_match_dict[position][direction]['match_length'] = None
    for position in positions_y:
        for direction in directions_y:
            if track_match_dict[position][direction]['match_length'] and match_dict[position][direction][
                'need_length']:
                if match_dict[position][direction]['need_length']- match_buffer <= track_match_dict[position][direction][
                    'match_length']:
                    done_dict[position] += 1  # 画好一条边
                    print('Done' + position)
                    print(done_dict)
                    track_match_dict[position][direction]['end_point'] = None
                    track_match_dict[position][direction]['match_length'] = None
        # match_dict[position][direction]['need_length'] = track_match_dict[position][direction]['need_length']
        # match_dict[position][direction]['end_point'] = track_match_dict[position][direction]['end_point']
        # done_dict[position] += 1  # 画好一条边
        # print('Done' + position)
        # print(done_dict)


def reset_direction_count(track_cur,out_direction_change_th):        # 重置方向计数
    if not track_cur:
        ...
        diff = np.diff(np.array(track_cur).T, 1).T
        if diff.size==0:
            diff = np.array([[0,0]])
        else:
            diff = np.concatenate((np.array([[0,0]]),np.diff(np.array(track_cur).T,1).T))
        fit_change_num_x_plus = np.sum(diff[:,0]>out_direction_change_th)
        fit_change_num_y_plus = np.sum(diff[:,1]>out_direction_change_th)
        fit_change_num_x_minus = np.sum(diff[:,0]<-out_direction_change_th)
        fit_change_num_y_minus = np.sum(diff[:,1]<-out_direction_change_th)
...
def reset():     #重置所有计数器及容器 —— 拟合一次后调用
    ...
    move_cur_img = []               # 被移除的轨迹点对应的图片——下一段轨迹的开头
    update_flag = False             # 长度匹配开关，分出一个轨迹段之后打开，匹配一次后关闭
    position = ''                   # 当前位置
    direction = ''                  # 当前轨迹方向
    change_direction = ''           # 轨迹点变化方向

    in_direction_count = 0              # 处于该方向的次数
    out_direction_count = 0             # 该方向确定之后，不在该方向的次数
    out_count = 0                       # 跑出有效区域个数
    out_direction_dectect_flag = False  # 其他方向计数开关

    track_with_two_position = []  #两个区域重合的点
    track_with_two_position_img = []  # 两个区域重合的点对应图片

    return track_cur,track_cur_img,move_cur,move_cur_img,update_flag,\
           position,direction,change_direction,in_direction_count,out_direction_count,\
           out_count,out_direction_dectect_flag,track_with_two_position,track_with_two_position_img

class Track_match(object):
    def __init__(self):
        super().__init__()
        self.threshold__init__()
        self.flag__init__()
        self.tmp__init__()
        self.context__init__()
        self.dict__init__()

    def threshold__init__(self):
        ...
        self.match_buffer = 40  # 边框长度与轨迹长度匹配扩展距离

    def flag__init__(self):
    ...
    def context__init__(self):
        self.image_path = None       # 该时刻图片文件名（后期改成图片）
        self.track_point = np.array([])      # 该时刻轨迹点
        self.track_point_per = np.array([])  # 上一时刻的轨迹点
        self.screen_contours_best = np.array([])
        self.box_buffer = None       # 外接矩形框
        self.box_in_buffer = None    # 内接矩形框
        self.track_minpoint_pro = np.array([])   #轨迹投影最近点
        self.track_maxpoint_pro = np.array([])   #轨迹投影最近远
        self.track_length_pro = 0                #轨迹投影长度
        self.position = ''  # 当前位置
    ...
    def dict__init__(self):
        self.match_dict = {'left':  # 边框匹配字典
            {
                'y_plus': {'func': self.fit_contours_vertical, 'end_point': None, 'need_length': None},
                'y_minus': {'func': self.fit_contours_vertical, 'end_point': None, 'need_length': None},
                'k_fit': None,
                'b_fit': None
            },
            'right':
            ...
            'up':
                {
                    'x_plus': {'func': self.fit_contours_horizontal, 'end_point': None, 'need_length': None},
                    'x_minus': {'func': self.fit_contours_horizontal, 'end_point': None, 'need_length': None},
                    'k_fit': None,
                    'b_fit': None
                },
        }
    ...
    def add_context(self,image_path,track_point,box_buffer,box_in_buffer,screen_contours_best):
        self.screen_contours_best = screen_contours_best
        self.image_path = image_path
        self.track_point = track_point
        self.box_buffer = box_buffer
        self.box_in_buffer = box_in_buffer
    def calculate_diff(self):
        if self.track_point_per.size!=0:
            self.diff_x = self.track_point[0] - self.track_point_per[0]
            self.diff_y = self.track_point[1] - self.track_point_per[1]
        else:
            self.diff_x = 0
            self.diff_y = 0
    def joint_flag(self):
        if self.cur_area_count == 2 and self.per_area_count == 1:
            self.area_in_flag = True
            self.area_cut_flag = False
        if self.cur_area_count == 1 and self.per_area_count == 2:  # 只有第一次跑出重叠区域，重叠区域分段的条件才开启
            self.area_cut_flag = True
            if self.init_notin_flag:
                self.area_in_flag = False
        if self.cur_area_count == 1:  # 只有第一次跑出重叠区域，重叠区域分段的条件才开启
            self.init_notin_flag = True
        if self.cur_area_count == 2 and self.init_notin_flag:
            self.track_with_two_position.append(self.track_point)
            self.track_with_two_position_img.append(self.image_path)
    def count_in_change_nums(self):
        if self.diff_x > self.out_direction_change_th:  # 往x正方向变化——分段找其他方向的结束点阈值
            self.change_direction = 'x_plus'
        if self.diff_x > self.fit_change_th:  # 往x正方向变化——分段找该方向阈值
            self.fit_change_num_x_plus += 1

        if self.diff_x < -self.out_direction_change_th:  # 往x负方向变化——分段找其他方向的结束点阈值
            self.change_direction = 'x_minus'
        if self.diff_x < -self.fit_change_th:  # 往x负方向变化——分段找该方向阈值
            self.fit_change_num_x_minus += 1

        if self.diff_y > self.out_direction_change_th:  # 往y正方向变化——分段找其他方向的结束点阈值
            self.change_direction = 'y_plus'
        if self.diff_y > self.fit_change_th:  # 往y正方向变化——分段找该方向阈值
            self.fit_change_num_y_plus += 1

        if self.diff_y < -self.out_direction_change_th:  # 往y负方向变化——分段找其他方向的结束点阈值
            self.change_direction = 'y_minus'
        if self.diff_y < -self.fit_change_th:  # 往y负方向变化——分段找该方向阈值
            self.fit_change_num_y_minus += 1

    def track_point_tmp_save(self):
        self.track_cur.append(self.track_point)
        self.track_cur_img.append(self.image_path)
        self.track_point_per = self.track_point
        self.per_area_count = self.cur_area_count

    def track_point_all_save(self):
        self.track_all.append(self.track_cur)
        self.track_img_all.append(self.track_cur_img)
        self.track_direction_all.append(self.direction)

    def count_out_change_nums(self):
        self.out_count += 1
        if self.per_area_count == 2:
            self.area_cut_flag = True
        print(self.track_point)
    def fit_and_projection(self):
            # 确定位置
            if 'y' in self.direction:
                self.position = self.find_position(self.track_cur[-2], self.box_buffer, self.box_in_buffer)  # 用有效轨迹的最后一个点判断位置，防止最后一个轨迹点是无效的
            else:
                self.position = 'up'
            if self.position:
                func = self.match_dict[self.position][self.direction]['func']
                end_point = self.match_dict[self.position][self.direction]['end_point']
                need_length = self.match_dict[self.position][self.direction]['need_length']
                k_track_fit, b_track_fit, track_true_length, track_minpoint, track_maxpoint = func(np.array(self.track_cur))
                (track_x_min, track_y_min) = track_minpoint
                (track_x_max, track_y_max) = track_maxpoint
                self.track_minpoint_pro, self.track_maxpoint_pro, self.track_length_pro = self.projection_length(track_minpoint, track_maxpoint,
                                                                                             self.match_dict[self.position]['k_fit'],
                                                                                             self.match_dict[self.position]['b_fit'])
                # (track_x_min_pro, track_y_min_pro) = track_minpoint_pro
                # (track_x_max_pro, track_y_max_pro) = track_maxpoint_pro

                print("真实轨迹长度：", track_true_length, "投影长度：", self.track_length_pro)
                # cv2.line(img_copy, self.track_minpoint_pro, self.track_maxpoint_pro, (255, 255, 0), 2)

    def update_length(self):
        (track_x_min_pro, track_y_min_pro) = self.track_minpoint_pro
        (track_x_max_pro, track_y_max_pro) = self.track_maxpoint_pro
        position = self.position
        direction = self.direction
        track_length_pro = self.track_length_pro
        if self.update_flag:
            if direction == 'x_plus':
                if self.track_match_dict[position][direction]['end_point']:
                    if track_x_max_pro > self.track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                        self.track_match_dict[position][direction]['end_point'] = track_x_max_pro
                        self.track_match_dict[position][direction]['match_length'] += track_length_pro
                else:
                    self.track_match_dict[position][direction]['end_point'] = track_x_max_pro  # 之前没有匹配过
                    self.track_match_dict[position][direction]['match_length'] = track_length_pro

            elif direction == 'x_minus':
                if self.track_match_dict[position][direction]['end_point']:
                    if track_x_min_pro < self.track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                        self.track_match_dict[position][direction]['end_point'] = track_x_min_pro
                        self.track_match_dict[position][direction]['match_length'] += track_length_pro
                else:
                    self.track_match_dict[position][direction]['end_point'] = track_x_min_pro  # 之前没有匹配过
                    self.track_match_dict[position][direction]['match_length'] = track_length_pro

            elif direction == 'y_plus':
                if self.track_match_dict[position][direction]['end_point']:
                    if track_y_max_pro > self.track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                        self.track_match_dict[position][direction]['end_point'] = track_y_max_pro
                        self.track_match_dict[position][direction]['match_length'] += track_length_pro
                else:
                    self.track_match_dict[position][direction]['end_point'] = track_y_max_pro
                    self.track_match_dict[position][direction]['match_length'] = track_length_pro
                ...
                if self.track_match_dict[position][direction]['end_point']:
                    if track_y_min_pro < self.track_match_dict[position][direction]['end_point']:  # 之前匹配过切比原来的远
                        self.track_match_dict[position][direction]['end_point'] = track_y_min_pro
                        self.track_match_dict[position][direction]['match_length'] += track_length_pro
                else:
                    self.track_match_dict[position][direction]['end_point'] = track_y_min_pro
                    self.track_match_dict[position][direction]['match_length'] = track_length_pro  # 同个方向的都拼一块了，没有判断起点终点！！！！！
    def move_cur_save(self):
        self.out_direction_count += 1
        self.move_cur.append(self.track_point)  # 被移除的轨迹点——下一段轨迹的开头
        self.move_cur_img.append(self.image_path)

    def move_cur_clear(self):
        self.out_direction_count = 0
        self.move_cur.clear()
        self.move_cur_img.clear()
        self.fit_change_num_x_plus, self.fit_change_num_y_plus, self.fit_change_num_x_minus, self.fit_change_num_y_minus = self.reset_direction_count(
            [], self.out_direction_change_th)
    def out_direction_dectect(self):
        if self.direction and self.is_in_area(self.track_point, self.box_buffer, self.box_in_buffer):
            self.out_direction_dectect_flag = True
            self.fit_change_num_x_plus, self.fit_change_num_y_plus, self.fit_change_num_x_minus, self.fit_change_num_y_minus = self.reset_direction_count(
                [], self.out_direction_change_th)
            if self.out_direction_dectect_flag:
                if abs(self.diff_x) > self.out_direction_change_th and abs(self.diff_y) > self.out_direction_change_th:  # 两个方向同时变就跳过去
                    self.move_cur_save()
                else:
                    if self.direction == self.change_direction:  # 后续出现该方向的其他点也归并到这一段里面
                        self.move_cur_clear()
                    else:
                        self.move_cur_save()
    def track_next_develop(self):
        move_cur_tmp = copy.deepcopy(self.move_cur)
        move_cur_img_tmp = copy.deepcopy(self.move_cur_img)

        if not self.area_cut_flag:  # 找到反向位置及反向后的无效轨迹点
            turn_index = self.find_turn_index(self.track_cur, self.direction)
            move_cur_tmp = self.track_cur[turn_index + 1:] + move_cur_tmp
            move_cur_img_tmp = self.track_cur_img[turn_index + 1:] + move_cur_img_tmp

        track_with_two_position_tmp = copy.deepcopy(self.track_with_two_position)
        track_with_two_position_img_tmp = copy.deepcopy(self.track_with_two_position_img)
        self.reset_config()
        if self.area_cut_flag:  # 由于进出重叠区域分段
            self.track_cur = self.track_cur + track_with_two_position_tmp
            self.track_cur_img = self.track_cur_img + track_with_two_position_img_tmp

        else:  # 由于反向分段
            self.track_cur = self.track_cur + move_cur_tmp
            self.track_cur_img = self.track_cur_img + move_cur_img_tmp
    def track_next_init(self):
        if self.cur_area_count == 2:  # 拟合完还是从重合区域开始的话，就当作是轨迹点刚开始进来一样
            self.init_notin_flag = False
        self.fit_change_num_x_plus, self.fit_change_num_y_plus, self.fit_change_num_x_minus, self.fit_change_num_y_minus = self.reset_direction_count(
                self.track_cur, self.out_direction_change_th)
        self.direction = self.find_direction_whenin(self.fit_change_num_x_plus, self.fit_change_num_y_plus, self.fit_change_num_x_minus,
                                          self.fit_change_num_y_minus, self.in_direction_count_th)

    # 轨迹点是否在有效区域内
    def is_in_area(self,point, box_buffer, box_in_buffer):  # 格式 np.array([x,y]):
    ...
    # 轨迹直线拟合
    def fit_contours_vertical(self,screen_contours_best_left_xy):  # 输入一边纵向轨迹，拟合直线; 格式 np.array([[x1,y1]，[x2,y2]...])
        block_left_y_max = screen_contours_best_left_xy[:, 1].max()
        block_left_y_min = screen_contours_best_left_xy[:, 1].min()
        # rows, cols = img_copy.shape[:2]
        [vx, vy, x_point, y_point] = cv2.fitLine(screen_contours_best_left_xy, cv2.DIST_L2, 0, 0.01, 0.01)
        ...
        if np.count_nonzero(k_fit) != 0:
            ...
            left_x_max = screen_contours_best_left_xy[:, 0].max()
            left_x_min = screen_contours_best_left_xy[:, 0].min()
        track_length = np.linalg.norm(
            np.array([left_x_min, block_left_y_min]) - np.array([left_x_max, block_left_y_max]))

        return k_fit, b_fit, track_length, (left_x_min, block_left_y_min), (left_x_max, block_left_y_max)

    def fit_contours_horizontal(self,screen_contours_best_up_xy):  # 输入一边横向轨迹，拟合直线; 格式 np.array([[x1,y1]，[x2,y2]...])
        block_up_x_max = screen_contours_best_up_xy[:, 0].max()
        block_up_x_min = screen_contours_best_up_xy[:, 0].min()
        # rows, cols = img_copy.shape[:2]
        [vx, vy, x_point, y_point] = cv2.fitLine(screen_contours_best_up_xy, cv2.DIST_L2, 0, 0.01, 0.01)
    ...
    def projection(self,point, k, b):  # 求投影后坐标
        x = (k * (point[1] - b) + point[0]) / (k ** 2 + 1)
        y = k * x + b
        return int(x), int(y)

    def projection_length(self,track_minpoint, track_maxpoint, k_fit, b_fit):  # 轨迹投影后坐标与长度
        track_left_x_min, track_left_y_min = projection(track_minpoint, k_fit, b_fit)
        track_left_x_max, track_left_y_max = projection(track_maxpoint, k_fit, b_fit)
        track_length_pro = np.linalg.norm(
            np.array([track_left_x_min, track_left_y_min]) - np.array([track_left_x_max, track_left_y_max]))
        track_minpoint_pro = (track_left_x_min, track_left_y_min)
        track_maxpoint_pro = (track_left_x_max, track_left_y_max)
        return track_minpoint_pro, track_maxpoint_pro, track_length_pro

    # 决策字典更新
    def update_match_dict(self,match_dict, position, direction, end, length):
        match_dict[position][direction]['end_point'] = end
        match_dict[position][direction]['need_length'] = length

    # 更新边框字典
    def fit_screen(self,match_dict, screen_contours_best, box_buffer, box_in_buffer):
        scb = screen_contours_best[0].squeeze()
        screen_contours_best_part_xy, screen_contours_best_part, screen_contours_best_part_length = dividing_contours(
            scb,
            box_buffer,
            box_in_buffer)
        (screen_contours_best_left_xy, screen_contours_best_right_xy,
        ...
        # print("边框周长：", contour_left_length)
        # 拟合曲线
        k_fit, b_fit, track_length, minpoint, maxpoint = self.fit_contours_vertical(screen_contours_best_left_xy)
        match_dict['left']['k_fit'] = k_fit
        match_dict['left']['b_fit'] = b_fit
        (left_x_min, left_y_min) = minpoint
        (left_x_max, left_y_max) = maxpoint
        ...
        k_fit_r, b_fit_r, track_length_r, minpoint_r, maxpoint_r = self.fit_contours_vertical(screen_contours_best_right_xy)
        # print("右边框长度：", track_length_r)
        match_dict['right']['k_fit'] = k_fit_r
        match_dict['right']['b_fit'] = b_fit_r
        (right_x_min, right_y_min) = minpoint_r
        ...
        k_fit_u, b_fit_u, track_length_u, minpoint_u, maxpoint_u = self.fit_contours_horizontal(screen_contours_best_up_xy)
        match_dict['up']['k_fit'] = k_fit_u
        match_dict['up']['b_fit'] = b_fit_u
        (up_x_min, up_y_min) = minpoint_u
        (up_x_max, up_y_max) = maxpoint_u
    ...
    # 出界时找轨迹变化方向,两个方向次数一样时怎么处理
    def find_direction_whenout(self,fit_change_num_x_minus, fit_change_num_x_plus, fit_change_num_y_minus,
                               fit_change_num_y_plus):
        direction_dict = {'x_minus': fit_change_num_x_minus, 'x_plus': fit_change_num_x_plus,
                          'y_minus': fit_change_num_y_minus, 'y_plus': fit_change_num_y_plus}
        max_num = max(fit_change_num_x_minus, fit_change_num_x_plus, fit_change_num_y_minus, fit_change_num_y_plus)
        direction_max = [k for k, v in direction_dict.items() if v == max_num]
    ...
    # 界内找轨迹变化方向
    def find_direction_whenin(self,fit_change_num_x_minus, fit_change_num_x_plus, fit_change_num_y_minus,
                              fit_change_num_y_plus, in_direction_count_th):
        if fit_change_num_x_minus >= in_direction_count_th:
            return 'x_minus'
        elif fit_change_num_x_plus >= in_direction_count_th:
            return 'x_plus'
    ...
    def find_turn_index(self,track_cur, direction):
        if direction == 'y_plus':
            index = np.where(np.array(track_cur)[:, 1] == np.array(track_cur)[:, 1].max())[0][-1]
        elif direction == 'x_plus':
            index = np.where(np.array(track_cur)[:, 0] == np.array(track_cur)[:, 0].max())[0][-1]
    ...
    # 确定轨迹点位置
    def find_position(self,point, box_buffer, box_in_buffer):
        x_inbuffer_left, x_inbuffer_right, y_inbuffer_up, y_inbuffer_down = box_in_buffer[0][0], box_in_buffer[1][0], \
                                                                            box_in_buffer[1][1], box_in_buffer[2][1]
        xbuffer_left, xbuffer_right, ybuffer_up, ybuffer_down = box_buffer[0][0], box_buffer[1][0], \
                                                                box_buffer[1][1], box_buffer[2][1]
    ...
    def count_position_area(self,point, box_buffer, box_in_buffer):
        x_inbuffer_left, x_inbuffer_right, y_inbuffer_up, y_inbuffer_down = box_in_buffer[0][0], box_in_buffer[1][0], \
                                                                            box_in_buffer[1][1], box_in_buffer[2][1]
        xbuffer_left, xbuffer_right, ybuffer_up, ybuffer_down = box_buffer[0][0], box_buffer[1][0], \
                                                                box_buffer[1][1], box_buffer[2][1]
    ...
    def isdone(self,match_dict: dict, track_match_dict: dict, done_dict: dict, match_buffer: int):
        #  判断长度
        positions_x = ['up']
        directions_x = ['x_plus', 'x_minus']
        positions_y = ['left', 'right']
        ...
        for position in positions_x:
            for direction in directions_x:
                if track_match_dict[position][direction]['match_length'] and match_dict[position][direction][
                    'need_length']:
                    if match_dict[position][direction]['need_length'] - match_buffer <= \
                            track_match_dict[position][direction]['match_length']:
                        done_dict[position] += 1  # 画好一条边
                        print('Done' + position)
                        print(done_dict)
                        track_match_dict[position][direction]['end_point'] = None
        ...
        for position in positions_y:
            for direction in directions_y:
                if track_match_dict[position][direction]['match_length'] and match_dict[position][direction][
                    'need_length']:
                    if match_dict[position][direction]['need_length'] - match_buffer <= \
                            track_match_dict[position][direction][
                                'match_length']:
                        done_dict[position] += 1  # 画好一条边
                        print('Done' + position)
                        print(done_dict)
    ...
    def reset_direction_count(self,track_cur, out_direction_change_th):  # 重置方向计数
        if not track_cur:
            fit_change_num_x_plus = 0  # 拟合点x正方向变化计数
            fit_change_num_y_plus = 0  # 拟合点y正方向变化计数
            fit_change_num_x_minus = 0  # 拟合点x负方向变化计数
    ...
    def main(self,image_path, track_point, box_buffer, box_in_buffer,screen_contours_best):
        self.add_context(image_path, track_point, box_buffer, box_in_buffer,screen_contours_best)
        fit_screen(self.match_dict, self.screen_contours_best, self.box_buffer, self.box_in_buffer)   #拟合边框直线并更新字典
        if not self.track_match_dict:
            self.track_match_dict = copy.deepcopy(self.match_dict)       #原始边框需要匹配的信息
        ...
        if is_in_area(self.track_point, self.box_buffer,self.box_in_buffer):  # 有效区域内

            self.cur_area_count = count_position_area(self.track_point,self.box_buffer,self.box_in_buffer)
            print("cur_area_count:", self.cur_area_count)
            # ---------------------------------------------------------------------------------------------------     joint
            self.joint_flag()
            # ---------------------------------------------------------------------------------------------------     joint
            # ---------------------------------------------------------------------------------------------------     count_in_change_nums
            self.count_in_change_nums()
            # ---------------------------------------------------------------------------------------------------     count_in_change_nums
            # ---------------------------------------------------------------------------------------------------     track_point_tmp_save
            self.track_point_tmp_save()
            # ---------------------------------------------------------------------------------------------------     track_point_tmp_save
        else:  # 有效区域外
            # ---------------------------------------------------------------------------------------------------     count_out_change_nums
            self.count_out_change_nums()
            # ---------------------------------------------------------------------------------------------------     count_out_change_nums
        # 分段
        if self.out_count >= self.out_count_th:
            self.direction = find_direction_whenout(self.fit_change_num_x_minus,self.fit_change_num_x_plus,self.fit_change_num_y_minus,self.fit_change_num_y_plus)
            print('direction:', self.direction, end=' ')

            # 有方向就拟合
            if self.direction and self.track_cur:
                self.update_flag = True
                # 确定位置
                # ---------------------------------------------------------------------------------------------------     fit_and_projecttion
                self.fit_and_projection()
            # ---------------------------------------------------------------------------------------------------     fit_and_projecttion
            # 更新
            # ---------------------------------------------------------------------------------------------------     update_length
            self.update_length()
            ...
            isdone(self.match_dict, self.track_match_dict, self.done_dict,self.match_buffer)
            # ---------------------------------------------------------------------------------------------------     track_point_all_save
            self.track_point_all_save()
            # ---------------------------------------------------------------------------------------------------     track_point_all_save
            self.reset_config()
        else:
            direction_tmp = find_direction_whenin(self.fit_change_num_x_minus,self.fit_change_num_x_plus,self.fit_change_num_y_minus,self.fit_change_num_y_plus,self.in_direction_count_th)
            if not self.out_direction_dectect_flag:
                self.direction = copy.deepcopy(direction_tmp)
            print('direction:', self.direction, end=' ')
            # ---------------------------------------------------------------------------------------------------     out_direction_dectect
            self.out_direction_dectect()
            # ---------------------------------------------------------------------------------------------------     out_direction_dectect
            if (self.area_cut_flag and not self.area_in_flag) or (
                    (self.out_direction_count >= self.out_direction_count_th) and self.direction):  # 反向或者进出重合区域就分段
                self.update_flag = True
                if not self.area_cut_flag:
                    self.track_cur = [point_xy for point_xy in self.track_cur if not np.isin(point_xy, self.move_cur).all()]  # 移除下一段的轨迹点
                    self.track_cur_img = [img_point for img_point in self.track_cur_img if img_point not in self.move_cur_img]
                # 确定位置
                # ---------------------------------------------------------------------------------------------------   fit_and_projection
                self.fit_and_projection()
                # ---------------------------------------------------------------------------------------------------   fit_and_projection
                ...
                # ---------------------------------------------------------------------------------------------------   track_next_develop
                self.track_next_develop()
                # ---------------------------------------------------------------------------------------------------   track_next_develop
                # ---------------------------------------------------------------------------------------------------   track_next_init
                self.track_next_init()
                # ---------------------------------------------------------------------------------------------------   track_next_init
            self.area_cut_flag = False
# # 创建新的图像并绘制分割轮廓
...
if __name__ == "__main__":
    ...
    def pngfilter(f):
        if f[-4:] in ['.jpg', '.png', '.bmp']:
            ...
            return False
    # data_root = os.getcwd() + '/exp14'
    # save_root = os.getcwd() + '/vis_img_exp14'
    # img_file_list_with_sub = os.listdir(data_root)
    # track = pd.read_csv("/hhd/Zhengzhuo/exp14/result_yifan.csv")

    data_root = os.getcwd() + '/data_310'
    save_root = os.getcwd() + '/vis_img_310_adapt'
    # img_file_list_with_sub = os.listdir(data_root)
    # track = pd.read_csv("/hhd/Zhengzhuo/result_yifan.csv")

    # data_root = os.getcwd() + '/dian_0314_left'
    # save_root = os.getcwd() + '/vis_img_dian_0314_left'
    #
    # data_root = os.getcwd() + '/jiaozheng'
    # save_root = os.getcwd() + '/vis_img_dian_jiaozheng'

    # data_root = os.getcwd() + '/rectify'
    # save_root = os.getcwd() + '/vis_img_rectify'

    track = pd.read_csv(data_root+"/result_yifan.csv")
    # track = pd.read_csv(data_root + "/path.csv")
    img_file_list_with_sub = os.listdir(data_root)
    ...
    track_match_process = Track_match()

    wait_time = 0.5
    i = 1
    break_for = False  #单张图片测试
    screen_contours_old = []
    screen_contours_new = []
    rect_all = []
    box_all = []
    ...
    buffer_in_right = 20  # 右内边框额外扩展像素点
    ...
    for image_path,track_point in zip(img_file_list,track_xy):
        ...
        # fig = plt.figure()
        # info = plt.imshow(mask,cmap='gray')
        ...
        # ---------------------------------------------------------------------------------------------------
        start = time.time()
        track_match_process.main(image_path,track_point,box_buffer,box_in_buffer,screen_contours_best)
        ...
        kernel_5x5 = np.ones((5, 5), np.uint8)
        kernel_3x3 = np.ones((3, 3), np.uint8)
        mask_test = cv2.dilate(mask, kernel_5x5, iterations=6)
        # opening = cv2.morphologyEx(gray_mask, cv2.MORPH_OPEN, kernel_5x5, iterations=10)
        ...
        # fig = plt.figure()
        # info = plt.imshow(gray_mask_ode,cmap='gray')
        # info = plt.imshow(combine, cmap='gray')
        print(i,image_path,'面积比：',area_per,'最大面积比:',area_per_min,'前后帧重心差距:',dist)
        # # # plt.pause(wait_time)
        # plt.show()
        ...
        if break_for:
            break
    print(track_match_process.done_dict)
        # 新的SN进来，轨迹置零，边框重置